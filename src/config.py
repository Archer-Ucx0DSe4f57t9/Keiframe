...
# 提醒配置：
ALERT_SOUND_COOLDOWN = 20 # 同名警告最短间隔（秒），低于间隔的音频不播放
...
ARTIFACT_READY_DETECTION_MODE = "not_idle"  # 可选值: "not_idle", "ready"
...
SUPPLY_SOUND_MIN_INTERVAL_SECONDS = 30 # 同名音频最短播放间隔（秒），低于间隔的播放请求将被忽略

SUPPLY_ALERT_OFFSET_X = 1800
SUPPLY_ALERT_OFFSET_Y = 40
SUPPLY_ALERT_WIDTH = 90 #消息框的最大宽度，字必须在这个宽度内显示完整
SUPPLY_ALERT_HEIGHT = 32 #消息框的高度
